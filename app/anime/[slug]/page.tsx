        ...
        <div className="flex-1 space-y-6">
          <div>
            ...
            <div className="flex flex-wrap gap-3 items-center text-sm font-medium text-white/60">
              {anime.rating && (
                <div className="flex items-center gap-1 text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded">
                  <Star className="w-4 h-4 fill-cyan-400" /> {anime.rating}
                </div>
              )}
              {anime.status && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {anime.episodes.map(ep => (
              <Link 
                ...
                className="flex items-center gap-4 p-3 rounded-xl border border-white/10 bg-[#0a0a0a] hover:border-cyan-400/50 transition-all group"
              >
                <div className="flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 text-white/60 font-bold group-hover:bg-cyan-500 group-hover:text-black transition-colors">
                  {ep.number || '?'}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {ep.date && <p className="text-xs text-white/40">{ep.date}</p>}
                </div>
              </Link>
            ))}
          </div>
